...
ID1 ()
 val  (exp as exp1) = exp1 ()
 in (
 { var = { name = S.symbol ID, escape = ref true }, typ = NONE, init = exp, pos = IDleft } 
)
end)
...
MlyValue.vardec (fn _ => let val  ID1 = ID1 ()
 ...
 val  (exp as exp1) = exp1 ()
 in (
 { var = { name = S.symbol ID1, escape = ref true }, typ = SOME (S.symbol ID2, ID2left), init = exp, pos = ID1left } 
)
end)
 in ( LrTable.NT 15, ( result, VAR1left, exp1right), rest671)
end
